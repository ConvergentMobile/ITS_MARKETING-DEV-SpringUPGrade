...
public class LibertyAdminDAOManager implements Serializable {
	...
	public List<CustomFields> getReservedKeywords(Long userId) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
                        Query q = session.createNativeQuery(sql,CustomFields.class)
			...
			List<CustomFields> sites = q.setParameter(0, userId)
							.list();
	...
	public List<CustomFields> getEntityKeyword(Long userId) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("cf", CustomFields.class);
			List<CustomFields> sites = q.setParameter(0, userId)
							.list();			
	...
	public List<ValueObject> getSAFOffices(Long userId) throws Exception {
		...
		try {
			...
			List<ValueObject> sites = q.setParameter(0, userId)	
										.list();			
	...
	public List<UserProfileVO> getSAFSites(Long userId) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql,UserProfileVO.class)
					.addScalar("userId", new LongType())
			...
			List<UserProfileVO> sites = q.setParameter(0, userId)
		...
		*/
		...
		try {
          ...
          */
			...
			List<UserProfileVO> sites = q.setParameter(0, letter+"%")
	...
	public List<UserProfileVO> getSites(String sortColumn, String sortOrder) throws Exception {
		...
		try {
			...
			List<UserProfileVO> sites = q.setParameter(0, siteId)
											.list();			
	...
	public List<TargetUserList> getLists(Long userId, String letter) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("tul", TargetUserList.class);
			List<TargetUserList> lists = q.setParameter(0, letter+"%").setParameter(1, userId).list();
	...
	public List<SendMessageList> getLists1(Long userId) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql,SendMessageList.class);
                                //.setResultTransformer(Transformers.aliasToBean(SendMessageList.class));
			lists = q.setParameter(0, userId)
						.list();			
	...
	public List<CustomFields> getCustomFields(Long userId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<CustomFields> cfields = q.setParameter(0, userId).list();
	...
	public List<CustomFields> getCustomFieldsByOffice(String id) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<CustomFields> cfields = q.setParameter(0, id).list();
	...
	public CustomFields getCustomFieldsByEntity(String entityId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<CustomFields> cfields = q.setParameter(0, entityId).list();
	...
	public User getUserByOfficeId(String officeId) throws Exception {
		...
		try {		
			...
			User user = (User) q.setParameter(0, officeId).list().get(0); //should be only one
	...
	public User getUserByEntityId(String entityId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("user", User.class);
			User user = (User) q.setParameter(0, entityId).list().get(0); //should be only one
	...
	public KeywordApplication getKeywordByOfficeId(String officeId) throws Exception {
		...
		try {		
			...
			List<KeywordApplication> kwAppls = (List<KeywordApplication>) q.setParameter(0, officeId)
	...
	public KeywordApplication getKeywordByEntityId(String entityId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("kw", KeywordApplication.class);
			List<KeywordApplication> kwAppls = (List<KeywordApplication>) q.setParameter(0, entityId)
								.list(); //should be only one
	...
	public KeywordApplication getKeywordByUserId(Long userId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("kw", KeywordApplication.class);
			List<KeywordApplication> kwAppls = (List<KeywordApplication>) q.setParameter(0, userId)
								.list(); //should be only one
	...
	public List<UserProfileVO> searchKeyword(Integer siteId, String searchKeywordStr, String searchOfficeIdStr, 
		...
		try {
			...
			List<UserProfileVO> kwList = q.setParameter("siteId", siteId)
											.setParameter("keyword", "%"+searchKeywordStr+"%")
											.setParameter("officeId", "%"+searchOfficeIdStr+"%")
											.setParameter("city", "%"+searchCityStr+"%")
											.setParameter("entityId", "%"+searchEntityIdStr+"%")
											.list();			
	...
	public List<ApprovedMessage> getApprovedMsgsCA(Integer siteId, String lang) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<ApprovedMessage> msgList = q.setParameter("siteId", siteId)
	...
	public List<ApprovedMessage> getCustomMsgs(Integer siteId, Long userId, String entType, String status, Integer days) throws Exception {
		...
		*/	
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql).addEntity("ap", ApprovedMessage.class);
	...
	public ApprovedMessage getCustomMsgByIdNA(Integer msgId) throws Exception {
		...
		try {		
			...
			List<ApprovedMessage> msgList = q.setParameter("msgId", msgId)
											.list();
			if (msgList.isEmpty())
				return null;
	...
	public List<ApprovedMessage> getApprovedMsgsAD(Integer siteId, Long userId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			msgList = q.setParameter(0, siteId).setParameter(1, userId).list();
	...
	public List<ApprovedMessage> getPendingMsgs(String eid) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			msgList = q.setParameter(0, eid).list();
	...
	public List<ApprovedMessage> getPendingMsgsAD(Integer siteId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			msgList = q.setParameter(0, siteId).list();
	...
	public void changeKW(String oldKeyword, String keyword, Integer siteId) throws Exception {
		...
		try {
			...
			List<UserProfileVO> uids = q.setParameter(0, oldKeyword)
										.setParameter(1, siteId)
										.list();			
			...
			q = session.createNativeQuery(usql1);	
			q.setParameter(0, keyword)
			  .setParameter(1, userId)
			  .setParameter(2, oldKeyword);					
			int ret = q.executeUpdate();			
			logger.debug(ret + " rows updated in user");
			
			q = session.createNativeQuery(usql2);	
			q.setParameter(0, keyword)
			 .setParameter(1, userId)
			 .setParameter(2, oldKeyword);					
			ret = q.executeUpdate();
			...
			q = session.createNativeQuery(usql3);	
			q.setParameter(0, keyword)
			 .setParameter(1, oldKeyword);					
			ret = q.executeUpdate();
	...
	public Territory getTerritory(String territoryId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<Territory> terrList = q.setParameter(0, territoryId).list();
	...
	public List<Territory> getTerritoryByMBEntity(String mbEntityId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<Territory> terrList = q.setParameter(0, mbEntityId).list();
	...
	public List<Territory> getTerritoryByEntity(String entityId) throws Exception {
		...
		try {		
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql);
			List<Territory> terrList = q.setParameter(0, entityId).list();
	...
	public void createAccountingADList(String listId, Map<String, String> adMap) throws Exception {
		...
		try {			
			session = HibernateUtil.currentSession();
			conn = HibernateUtil.getConnection(session);
