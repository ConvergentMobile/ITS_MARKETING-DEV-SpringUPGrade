...
public class LTUserDAOManager extends UserDAOManager {
	...
	public LTCategory_3 getProfile(Long userId, String sql) throws Exception {
		try {
			...
			List<LTCategory_3> profiles = q.setParameter(0, userId).list();
	...
	public List<RoleAction> getRoleActions(String role) throws Exception {
		...
		try {
			session = HibernateUtil.currentSession();
			Query q = session.createNativeQuery(sql,RoleAction.class)
					.addScalar("roleAction", new StringType())					
					.addScalar("roleActionUrl", new StringType())
					.addScalar("roleType", new StringType());
                                        
					//q.setResultTransformer(Transformers.aliasToBean(RoleAction.class));	
			
			List<RoleAction> roleActions = q.setParameter("role", role)
	...
	public List<ValueObject> getMsgQuota(Long userId) throws Exception {
		...
		try {
			...
			List<ValueObject> quota = q.setParameter("userId", userId).list();
	...
	public List<ValueObject> getAllOffices(Long userId) throws Exception {
		...
		try {
			...
			Query q = session.createNativeQuery(sql,ValueObject.class)
					...
					//.setResultTransformer(Transformers.aliasToBean(ValueObject.class));							
			List<ValueObject> res = q.setParameter("parentId", userId).list();
	...
	public List<ValueObject> getListData(String listId) throws Exception {
		...
		try {
			...
			List<ValueObject> listData = q.setParameter("listId", listId).list();
	...
	public int optout(String mobilePhone, String shortcode, String keyword) throws Exception {
		...
		try {
			...
			int idx = 0;
			q.setParameter(idx++, shortcode)
			  .setParameter(idx++, mobilePhone)
			  .setParameter(idx++, mobilePhone.substring(1));
			
			int ret = q.executeUpdate();
			
			q = session.createNativeQuery(deleteSql);
			idx = 0;
			q.setParameter(idx++, mobilePhone)
				.setParameter(idx++, mobilePhone.substring(1));
	...
	public int optout(String mobilePhone, String shortcode, String keyword) throws Exception {
		...
		try {
			...
			int idx = 0;
			q.setParameter(idx++, mobilePhone)
			  .setParameter(idx++, shortcode)
			  .setParameter(idx++, keyword)
			  .setParameter(idx++, keyword)
			  .setParameter(idx++, mobilePhone)
			  .setParameter(idx++, mobilePhone.substring(1))
			  .setParameter(idx++, keyword)
